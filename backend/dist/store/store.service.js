...
let StoreService = class StoreService {
    ...
    getServiceById(serviceId) {
        return this.serializeService(this.requireService(serviceId));
    }
    ...
    createCase(actor, payload) {
        ...
        const caseRecord = this.createCaseRecord({
            ...
            title: payload.title,
            description: payload.description,
            status: entities_1.CaseStatus.OPEN,
        ...
        const customer = this.requireUser(booking.customerId);
        const provider = this.requireUser(booking.providerId);
        [...adminUsers, customer, provider].forEach((user) => {
            if (user.id === actor.id && user.role !== entities_1.Role.ADMIN)
                return;
            this.appendNotification(user.id, 'New dispute case', `${payload.title} has been opened for booking ${booking.id}.`, entities_1.NotificationTone.WARNING, 'case', caseRecord.id);
        });
        return this.serializeCase(caseRecord);
    }
